...
const GetRequests = () => {
  ...
  const handleInvoice = async (id) => {
    try {
      ...
      doc.text(generatedDate, pageWidth - marginRight, marginTop, {
        align: "right",
      });
      ...
      doc.text(
        `Request Date: ${new Date(
          request.data.request.createdAt
        ).toLocaleDateString()}`,
        10,
        60
      );
      doc.text(
        `Hospital Name: ${request.data.request.requestHospitalName}`,
        10,
        70
      );
      doc.text(
        `Blood Type: ${request.data.request.bloodType}${
          request.rhFactor === "positive" ? "+" : "-"
        }`,
        10,
        80
      ...
      doc.text(
        `Physician Name: ${request.data.request.physicianName}`,
        10,
        100
      );
      doc.text(
        `Physician Phone: ${request.data.request.physicianPhone}`,
        10,
        110
      );
      doc.text(
  ...
  const filteredData = requestData.filter((request) =>
    request.requestHospitalName.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="container">
      {requestData.length === 0 ? (
        ...
        <div>
          ...
          <button
            className="btn btn-primary mb-3 border"
              ...
              {filteredData.map((request, index) => (
                <tr key={index}>
                  ...
                  <td>
                    ...
                    <Link
                      to={`/${request._id}/update`}
                      className="btn btn-body border border-dark me-2"
                    >
                      Update
                    </Link>
                    <button
